        ...
        if len(cts):
            ...
            if movs_of is not None:
                ...
                if len(mine):
                    ...
                    if over > 1.0:
                        print("  ⚠ 슬롯 넘침   %.1f초 (이 악장의 마지막 화음 %.1f초)"
                              % (over, mine[-1]))
                        # **넘침이 늘 결함은 아니다** (`05` 9.23절).
                        # 화음 하나의 여운이 넘는 것과 진행이 통째로 넘는 것은
                        # 다르다. 앞은 두 악장을 잇고(6→7 이 그 예다), 뒤는
                        # 두 템포를 겹쳐 울려 둘 다 흐린다(1·7악장이 그랬다).
                        # **탐지기는 그 구분을 못 한다. 사람이 판단한다.**
                        print("              몇 초인가로 갈린다 — 여운 하나(2~3초)면 "
                              "이음매일 수 있고, 진행이 통째로 넘었으면 결함이다")
        if wav is not None:
