...
def main():
    ...
    if r.returncode != 0 or not body.strip():
        ...
        print("=" * 66)
        # ── 마감이면 24비트도 함께 나가야 한다 (2026-08-13) ──────────────
        #
        # **검수자 판정** — *"일일마감할 때 최종 9악장 음원의 경우 24비트도
        # 동시에 찍어내서 google drive까지 업로드함."*
        #
        # **그 판정을 받은 날의 마감에서 내가 그대로 빠뜨렸다.** 전곡을
        # 32비트로만 굽고 넘어갔고, 검수자가 *"이것 24bit 버전 파일
        # 안만든것 같은데?"* 라고 찾아냈다.
        #
        # **규칙을 문서에만 적으면 안 지켜진다** — 오늘 구간표에서 같은 일이
        # 네 번째로 났고, 그래서 도구가 말하게 했다. 여기도 같다.
        if len(bits_list) == 1:
            print("⚠ 32비트 하나만 구웠다. **마감 파일이면 이것으로 부족하다** —")
            print("   `--bits both` 로 다시 굽는다 (24비트도 Drive 에 함께 올린다).")
            print("-" * 66)
